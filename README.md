...
1. **bang** - Play/pause the currently loaded track.
	- Sends `play (1|0)` through the last outlet to indicate whether it's playing or paused.
...
3. **list** - Outputs various messages including information regarding whether a file was successfully opened, whether a track is currently playing, track metadata, etc. Usually in `key: value` form.
...
- `[open $1 (` - Attempts to open a file.
	- Sends `open (1|0)` through the last outlet to indicate success or failure.
...
- `[conv $1 (` - Change the interpolation algorithm.
	- Options include:
...
- `[open $1 (` - Attempts to open a file.
	- Responds with `open (1|0)` through the last outlet to indicate success or failure.
...
1. **float** - The midi note of the scale index

2. **list** - Slices and alterations of scales

### Messages

- `[print (` - Print the current state of the scale.
	- A label is optional to help determine where the print request came from.

- `[n $1 (` - Set the size of the scale.
...
- `[send $.. (` - Send an alteration of the scale through the 2nd outlet without actually changing the internal scale.
...
1. **symbol** - The name of an existing garray.
...
2. **float** - Precision level
	- Max value is `ceil(mant_digits / log2(base))` with `mant_digits` being mantissa bits + 1 for the implied extra digit.
	- Zero sets to the default precision for the given base. Default is not the same as max.
...
5. **float** - Pixels per step (X axis)
	- Higher value lowers sensitivity of dragging. Zero disables the axis.

6. **float** - Pixels per step (Y axis)
	- Higher value lowers sensitivity of dragging. Zero disables the axis.

7. **float** - Character width
	- Default: zero.

8. **float** - Font size
	- Default: auto

9. **symbol** - Receive symbol

10. **symbol** - Send symbol

11. **symbol** - Label

12. **float** - Label position
...
- `[param $.. (` - Set all parameters.
	- Intended more for the dialog's response.
...
1. **float** - Starting value.

### Inlets

1. **bang** - Output current value.

2. **float** - Incoming value.
...
1. **float** - Control value.

### Inlets

1. **float** - Incoming values.

2. **float** - Changes the control value.

### Outlets

1. **float** - Values greater than control value.

2. **float** - Values less than or equal to control value.
