#include "hot.h"

/* -------------------------- hot multiplication -------------------------- */

static t_class *htimes_class;
static t_class *htimes_proxy_class;

static void htimes_bang(t_hot *x) {
	outlet_float(x->x_obj.ob_outlet, x->x_f1 * x->x_f2);
}

static void *htimes_new(t_symbol *s, int ac, t_atom *av) {
	return (hot_new(htimes_class, htimes_proxy_class, htimes_bang, s, ac, av));
}

void setup_0x230x2a(void) {
	htimes_class = class_new(gensym("#*"),
		(t_newmethod)htimes_new, (t_method)hot_free,
		sizeof(t_hot), 0,
		A_GIMME, 0);
	class_addbang(htimes_class, htimes_bang);
	class_addfloat(htimes_class, hot_float);
	class_addmethod(htimes_class, (t_method)hot_loadbang,
		gensym("loadbang"), A_DEFFLOAT, 0);

	htimes_proxy_class = class_new(gensym("_#*_proxy"), 0, 0,
		sizeof(t_hot_proxy), CLASS_PD | CLASS_NOINLET, 0);
	class_addbang(htimes_proxy_class, hot_proxy_bang);
	class_addfloat(htimes_proxy_class, hot_proxy_float);

	class_sethelpsymbol(htimes_class, gensym("hotbinops1"));
}
