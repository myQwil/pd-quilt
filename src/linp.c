...
static void linp_freeze(t_linp *x) {
	if (clock_getsystime() >= x->targettime) {
		...
		x->setval += x->invtime * (clock_getsystime() - x->prevtime)
...
static void linp_pause(t_linp *x, t_symbol *s, int ac, t_atom *av) {
	(void)s;
	if (x->setval == x->targetval || pause_state(&x->pause, ac, av)) {
		return;
	}
...
static void linp_float(t_linp *x, t_float f) {
	...
	if (x->gotinlet && x->in1val > 0) {
		if (timenow > x->targettime) {
			...
			x->setval = x->setval + x->invtime * (timenow - x->prevtime)
				* (x->targetval - x->setval);
		}
		x->prevtime = timenow;
