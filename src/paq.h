...
static t_paq *new_paq(t_class *cl, t_class *pxy
, t_symbol *s, int ac, t_atom *av, int r) {
	(void)s;
...
static const char *paq_check(t_atomtype type) {
	if (type == A_FLOAT) {
		return s_float.s_name;
	} else if (type == A_SYMBOL) {
		return s_symbol.s_name;
	} else if (type == A_POINTER) {
		return s_pointer.s_name;
	} else {
		return "null";
	}
}
...
static void paq_error(t_paq *x, int i, const char *t) {
	if (i) {
		pd_error(x, "inlet: expected '%s' but got '%s'", paq_check(x->type[i]), t);
	} else {
		pd_error(x, "paq_%s: wrong type", t);
	}
}
...
static void paq_bang(t_paq *x) {
	...
	for (int i = x->n; i--; vp++, tp++) {
		if (*tp == A_GIMME && vp->a_type != A_POINTER) {
			gp++;
		} else if (vp->a_type == A_POINTER) {
			if (!gpointer_check(gp, 1)) {
				return pd_error(x, "%s: stale pointer %d", class_getname(*(t_pd *)x), i);
			} else {
				gp++;
			}
		}
	...
	if (!x->outvec) {	/* LATER figure out how to deal with reentrancy and pointers... */
		if (x->nptr) {
...
static inline void paq_p(t_paq *x, t_gpointer *ptr, t_gpointer *gp, int i) {
	...
	if (type == A_POINTER || type == A_GIMME) {
		...
		*ptr = *gp;
		...
		paq_error(x, i, s_pointer.s_name);
	}
}
...
static void paq_anything(t_paq *x, t_symbol *s, int ac, t_atom *av) {
	if (paq_a(x, s, ac, av, PAQ_FIRST(x))) {
		paq_bang(x);
	}
}
