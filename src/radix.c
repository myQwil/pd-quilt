...
static void *radix_new(t_symbol *s, int argc, t_atom *argv) {
	...
	if (argc == 21) {
		log_height = atom_getfloatarg(20, argc, argv);
	}
	...
	case 2:
		strcpy(x->x_gui.x_font, "times"); break;
	case 1:
		strcpy(x->x_gui.x_font, "helvetica"); break;
	default:
...
void radix_setup(void) {
	...
	, (t_newmethod)radix_new, (t_method)radix_free
	, sizeof(t_radixtcl), 0
	, A_GIMME, 0);
