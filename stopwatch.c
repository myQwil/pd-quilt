...
static void stopwatch_push(t_stopwatch *x ,t_floatarg f) {
	x->setmore += f;
}

static void stopwatch_start(t_stopwatch *x ,t_floatarg f) {
	stopwatch_bang(x);
	stopwatch_push(x ,f);
}
...
void stopwatch_setup(void) {
	...
	class_addmethod(stopwatch_class ,(t_method)stopwatch_lap
		,gensym("lap")   ,0);
	class_addmethod(stopwatch_class ,(t_method)stopwatch_push
		,gensym("push")  ,A_FLOAT    ,0);
	class_addmethod(stopwatch_class ,(t_method)stopwatch_start
